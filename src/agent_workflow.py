...
from src.session_store import (
    SESSION_DIR,
    load_session as _shared_load_session,
    save_session as _shared_save_session,
)
from src.task_orchestration import (
    apply_privilege_prefix,
...
def _load_session(session_id: str) -> Dict[str, Any]:
    return _shared_load_session(session_id)


def _save_session(session_id: str, state: Dict[str, Any]):
    _shared_save_session(session_id, state)
...
def _extract_single_intent(text: str) -> Dict[str, Any]:
    ...
    if create_user_match:
        return {"intent": "create_user", "parameters": {"username": create_user_match.group(1)}}
    ...
    if delete_user_match:
        return {"intent": "delete_user", "parameters": {"username": delete_user_match.group(1)}}

    delete_path_match = re.search(r"(/[A-Za-z0-9_./-]+)(?![A-Za-z0-9_./-])", text)
    if delete_path_match and any(keyword in text_lower for keyword in ["删除", "移除", "remove", "delete", "rm"]):
        return {
            "intent": "delete_path",
            "parameters": {"path": delete_path_match.group(1), "recursive": True},
        }
...
def identify_intent(state: AgentState) -> Dict[str, Any]:
    ...
    if _is_confirmation_resume(state):
        ...
        return {
            ...
            "resolved_parameters": state.get("resolved_parameters", current_task.get("parameters", {})),
            "approval_active": state.get("approval_active", False),
    ...
    task_sequence = build_specialized_task_sequence(user_input, os_type)
    ...
    validated = StateValidator.validate_state({
        **state,
        ...
        "branch_results": {},
        "task_outputs": {},
        "resolved_parameters": task_sequence[0].get("parameters", {}) if task_sequence else {},
        "approval_active": False,
    })
...
def _build_task_sequence(intents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for idx, intent_item in enumerate(intents):
        task = TaskItem(
            ...
            status="pending",
            depends_on=[task_sequence[-1]["task_id"]] if task_sequence else [],
...
def pre_check_task(state: AgentState) -> Dict[str, Any]:
    ...
    resolved_parameters = resolve_template_value(task.get("parameters", {}), task_outputs)
    dependencies_ready, blockers = evaluate_dependency_state(task_sequence, task)
    if not dependencies_ready:
        task_sequence_copy = list(task_sequence)
        result_text = f"Task blocked by unresolved dependencies: {', '.join(blockers)}"
        task_sequence_copy[idx] = {**task_sequence_copy[idx], "status": "blocked", "result": result_text}
        next_idx = _get_next_task_index(state, idx)
        is_critical = task.get("is_critical", True)
        return {
            "task_sequence": task_sequence_copy,
            "current_task_index": next_idx,
            "task_status": "failed" if is_critical else ("in_progress" if next_idx < len(task_sequence_copy) else "completed"),
            "execution_result": result_text,
            "pre_check_passed": False,
            "abort_execution": is_critical,
            "skip_to_next": not is_critical,
            "resolved_parameters": resolved_parameters,
        }

    privilege_context = get_privilege_context(os_type)
    if task.get("intent") in {"create_user", "delete_user", "set_login_shell", "delete_path", "ensure_home_directory", "write_file", "set_permissions", "configure_sudo"}:
        if not privilege_context.get("is_root") and not privilege_context.get("passwordless_sudo"):
            result_text = "Privilege escalation required for this task, but passwordless sudo is unavailable."
            task_sequence_copy = list(task_sequence)
            task_sequence_copy[idx] = {**task_sequence_copy[idx], "status": "failed", "result": result_text}
            return {
                "task_sequence": task_sequence_copy,
                "task_status": "failed",
                "execution_result": result_text,
    ...
    pre_check = resolve_template_value(task.get("pre_check"), task_outputs)

    if not pre_check:
        return {"pre_check_passed": True, "resolved_parameters": resolved_parameters}
    ...
    try:
        ...
        return {"pre_check_passed": passed, "resolved_parameters": resolved_parameters}
    except Exception as e:
        return {"pre_check_passed": False, "pre_check_error": str(e), "resolved_parameters": resolved_parameters}
...
def generate_command(state: AgentState) -> Dict[str, Any]:
    ...
    if _is_confirmation_resume(state) and len(task_sequence) == 1:
        ...
        return {
            ...
            "resolved_parameters": task_sequence[idx].get("parameters", {}) if idx < len(task_sequence) else {},
        }
    ...
    params = state.get("resolved_parameters") or resolve_template_value(
        task.get("parameters", {}),
    ...
    if intent == "search_files":
        ...
        home_directory = params.get("home_directory", "")
        if os_type == "windows":
            command = f'net user {_safe_arg(username, os_type)} /add'
        else:
            base_cmd = f"useradd -m {_safe_arg(username, os_type)}"
            if home_directory:
                base_cmd = f"useradd -m -d {_safe_arg(home_directory, os_type)} {_safe_arg(username, os_type)}"
            if password:
                command = f"{base_cmd} && echo '{_safe_arg(username, os_type)}:{_safe_arg(password, os_type)}' | chpasswd"
            else:
                command = base_cmd
        ...
        command = f"usermod -s {_safe_arg(shell, os_type)} {_safe_arg(username, os_type)}"
    elif intent == "delete_path":
        path = params.get("path", "")
        recursive = params.get("recursive", True)
        command = f"rm -rf {_safe_arg(path, os_type)}" if recursive else f"rm -f {_safe_arg(path, os_type)}"
    elif intent == "ensure_home_directory":
        path = params.get("path", "")
        owner = params.get("owner", "")
        mode = params.get("mode", "755")
        command = f"mkdir -p {_safe_arg(path, os_type)} && chmod {_safe_arg(mode, os_type)} {_safe_arg(path, os_type)}"
        if owner:
            command += f" && chown {_safe_arg(owner, os_type)}:{_safe_arg(owner, os_type)} {_safe_arg(path, os_type)}"
    elif intent == "write_file":
        path = params.get("path", "")
        content = params.get("content", "")
        owner = params.get("owner", "")
        mode = params.get("mode", "")
        command = f"printf %s {_safe_arg(content, os_type)} > {_safe_arg(path, os_type)}"
        if owner:
            command += f" && chown {_safe_arg(owner, os_type)}:{_safe_arg(owner, os_type)} {_safe_arg(path, os_type)}"
        if mode:
            command += f" && chmod {_safe_arg(mode, os_type)} {_safe_arg(path, os_type)}"
    ...
    command, privilege_context = apply_privilege_prefix(command, task, params, os_type)
    ...
    if risk.risk_level == "medium" and not state.get("approval_active", False):
...
def execute_command(state: AgentState) -> Dict[str, Any]:
    ...
    execution_log = state.get("execution_log", [])
    task_outputs = state.get("task_outputs", {})
    resolved_parameters = state.get("resolved_parameters") or task.get("parameters", {})
    ...
    for attempt in range(max_retries):
        try:
            ...
            if is_non_retryable_failure(command, result.get("stderr", "")):
                break
        ...
        if ExecutionVerifier.has_verification(intent):
            try:
                ...
                if verify_res:
                    ...
                    if not success and verify_res.passed and contract.get("expect") != "nonzero":
                        task_status = "completed"
                        success = True
                        result_text += "Operation reached the expected state after verification.\n"
    ...
    if success:
        task_outputs = {
            **task_outputs,
            task.get("task_id", f"task_{idx}"): build_task_outputs(task, resolved_parameters, os_type),
        }
...
def handle_confirmation(state: AgentState) -> Dict[str, Any]:
    ...
    if user_confirmed:
        return {
            ...
            "approval_active": len(task_sequence) > 1,
        }
    else:
